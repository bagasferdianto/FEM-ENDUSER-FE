...
const TicketModal: React.FC<TicketModalProps> = ({ open, onClose, qrValue, ticketDate, used }) => {
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="flex flex-col items-center gap-4 p-6 rounded-2xl shadow-xl max-w-sm mx-auto w-fit px-24">
        ...
        <div className="flex flex-row text-sm text-gray-700 text-center">Surakarta,&nbsp;<div className='font-semibold'>{ticketDate}</div></div>
        
        <div className={`text-sm font-medium ${used ? 'text-red-500' : 'text-blue-pfl'}`}>
